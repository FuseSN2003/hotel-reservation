              ...
              <React.Fragment key={`roomTypes-${index}`}>
                ...
                >
                  ...
                  {Array.from({ length: daysArray.length }).map((_, i) => (
                    ...
                    >
                      {checkAvailableRooms(i + 1, roomTypes, searchCustomer) ===
                      0 ? (
                        <p
                ...
                {rooms.map((room: Room) => {
                  const thisRoomReservations = reservationData?.filter(
                    (reservation: any) => reservation.room_id === room.id
                  );
                        ...
                        for (let i = 0; i < daysArray.length; i++) {
                          ...
                          const thisColDate = new Date(
                            `${selectedYear}-${String(selectedMonth).padStart(
                              2,
                              '0'
                            )}-${String(thisColDays).padStart(2, '0')}`
                          );
                          ...
                          const thisReservation = thisRoomReservations?.find(
                            (reservation: any) =>
                              new Date(reservation.check_in) <= thisColDate &&
                              new Date(reservation.check_out) >= thisColDate
                          );
                          ...
                          if (thisReservation) {
                            ...
                            const timeDiff =
                              checkOutDate.getTime() - thisColDate.getTime();
                            const dayDiff = Math.ceil(
                              timeDiff / (1000 * 3600 * 24)
                            );

                            const nextReservation = thisRoomReservations?.find(
                              (reservation: Reservation) =>
                                new Date(reservation.check_in).getTime() ===
                                  checkOutDate.getTime() &&
                                reservation.room_id === thisReservation.room_id
                            );
                            ...
                            cells.push(
                              ...
                              >
                                <Reservation_detail_modal
                                  thisReservation={thisReservation}
                                  isOverflowFromPreviousMonth={
                                    isOverflowFromPreviousMonth
                                  }
                                  isOverflowToNextMonth={isOverflowToNextMonth}
